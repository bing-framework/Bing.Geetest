    ...
    public class GeetestManager : IGeetestManager
    {
        ...
        public async Task<GeetestRegisterResult> Register(string userId = null, string clientType = "unknown", string ipAddress = "unknown")
        {
            ...
            var url = string.IsNullOrWhiteSpace(userId)
                ? $"{Const.ApiUrl}{Const.RegisterUrl}?gt={config.Id}&client_type={clientType}&ip_address={ipAddress}"
                : $"{Const.ApiUrl}{Const.RegisterUrl}?gt={config.Id}&user_id={userId}&client_type={clientType}&ip_address={ipAddress}";
        ...
        /// <returns></returns>
        public async Task<bool> ValidateAsync(GeetestValidateParam parameter)
        {
            ...
            if (parameter.Validate.Length > 0 && parameter.Validate == Md5($"{config.Key}geetest{parameter.Challenge}"))
            {
                ...
                query += $"&sdk=csharp_{Const.Version}";
                var response = string.Empty;
                try
                {
                    var url = $"{Const.ApiUrl}{Const.ValidateUrl}";
